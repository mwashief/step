...
using umii = vector<pair<int, pair<int, base>>>;
...
void insert(umii *a, int k, pair<int, base> b)
{
    if (a->size() == 0)
    {
        a->pb(mp(k, b));
...
inline void __run_test()
{
    ...
    rrep(i, n)
    {
        ...
        for (auto d : *possible)
        {
            ...
            {
                if (i)
                    insert(temp, min(a[i], a[i - 1]), d.sc);
                res += base(d.sc.sc);
            }
            else
            {
                ...
                if (i)
                    insert(temp, min(a[i - 1], a[i] / divs), make_pair(d.sc.fr, d.sc.sc + base(d.sc.fr) * base(divs - 1)));
            }
        }
        insert(temp, min(a[i], a[i - 1]), mp(1, base(0)));
        swap(possible, temp);
    }
    cout << res << endl;
}
