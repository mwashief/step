/*
    ...
    11 December 2021
    Codeforces 1501-1750 1540 1540B
*/
...
base dp(int a, int b)
{
    ...
    if (b == 0)
        return base(0);
    if (memo[a][b] != -1)
        return base(memo[a][b]);
    base res = dp(a - 1, b) + dp(a, b - 1);
    res /= two;
    memo[a][b] = res.x;
    return res;
}
...
int main()
{
    ...
    rep(i, n) for (int j = i + 1; j < n; j++) rep(k, n)
    {
        if (adj[k][j] + adj[j][i] == adj[k][i])
            res += 1;
        else if (adj[k][i] + adj[i][j] != adj[k][j])
        {
            int dis = (adj[k][i] + adj[k][j] - adj[i][j]) >> 1;
            res += dp(adj[k][j] - dis, adj[k][i] - dis);
        }
    }
    res = res / base(n);
    cout << res << endl;
