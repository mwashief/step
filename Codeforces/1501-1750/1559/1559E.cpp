/*
    ...
    17 September 2021
    Codeforces 1501-1750 1559 1559E
*/
    ...
    auto dp = [&](int d)
    {
        ...
        rep(i, n)
        {
            ...
            rep(j, m + 1)
            {
                ...
                if (s > sum || start > finish)
                    break;
                temp[e] += memo[j];
                temp[s - 1] -= memo[j];
            }
            rrep(j, sum) temp[j] += temp[j + 1];
            memo = temp;
        }
        base res = 0;
        rep(i, sum + 1) res += memo[i];
        return res;
    };

    base res = 0;
    repe(i, m) if (miu[i]) res += base(miu[i]) * dp(i);
    cout << res << endl;
}