/*
    ...
    29 December 2021
    Codeforces 1501-1750 1616 1616E
*/
...
inline void __run_test()
{
    ...
    cin >> s >> t;
    vvi letters(26);
    rep(i, n) letters[s[i] - 'a'].pb(i);
    rep(i, 26) reverse(a2z(letters[i]));
    ll res = INT64_MAX;
    ll extra = 0;
    ...
    {
        int c = t[i] - 'a';
        rep(j, c) if (gsz(letters[j]))
            res = min<ll>(res, extra + letters[j].back() + st.query(letters[j].back(), letters[j].back()) - i);
        if (gsz(letters[c]))
        {
            int d = letters[c].back();
            letters[c].pop_back();
            extra += d + st.query(d, d) - i;
            st.update(0, d, 1);
        }
        else
            break;
