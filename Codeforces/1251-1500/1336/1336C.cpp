/*
    ...
    07 November 2021
    Codeforces 1251-1500 1336 1336C
*/
...
int main()
{
    ...
    string s, t;
    cin >> s >> t;
    vvbase memo(gsz(t) + 2, vbase(gsz(t) + 2));
    rep(i, gsz(t) + 2) memo[i][0] = 1;
    base res = 0;
    rep(i, gsz(s)) rep(j, gsz(t)) if (s[i] == t[j])
    {
        //place in front
        base temp = two.pow(i);
        if (j != gsz(t) - 1)
            temp = memo[j + 1][min(i, gsz(t) - j - 1)];
        memo[j][min(i + 1, gsz(t) - j)] += temp;
        if (j == 0 && min(i + 1, gsz(t) - j) == gsz(t))
            res += temp * base(gsz(s) - i);
