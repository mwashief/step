/*
    08 January 2023
    Codeforces 1751-2000 1783 1783D
*/
...
int main()
{
    ...
    int MAX = accumulate(a2z(a), 0) - a[0] + 1;

    vvl memo(n, vl(2 * MAX + 5, 0));
    memo[1][a[1] + MAX] = 1;
    ...
    for (int i = 2; i < n; i++)
        ...
        {
            int k = j + MAX;
            if (j - a[i] >= -MAX && j - a[i] <= MAX)
                memo[i][k] = memo[i - 1][j - a[i] + MAX];
            if (a[i] - j >= -MAX && a[i] - j <= MAX && j != a[i])
                memo[i][k] += memo[i - 1][a[i] - j + MAX];
