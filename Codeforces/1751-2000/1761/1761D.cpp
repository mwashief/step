/*
    ...
    24 November 2022
    Codeforces 1751-2000 1761 1761D
*/
...
int main()
{
    ...
    repe(i, k)
    {
        int rem = n - 2 * i;
        int vitore = k - i;
        int bahire = n - 2 * i - vitore;
        if (rem >= 0)
            res += base(3).pow(rem) * comb(i + bahire, i) * comb(vitore + i - 1, vitore);
        rem++;
        bahire++;
        if (rem >= 0)
            res += base(3).pow(rem) * comb(i + bahire - 1, bahire) * comb(vitore + i - 1, vitore);
    }
