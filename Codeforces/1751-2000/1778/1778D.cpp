/*
    01 February 2023
    Codeforces 1751-2000 1778 1778D
*/
...
inline void __run_test()
{
    ...
    cin >> a >> b;
    int dis = 0;
    rep(i, n) if (a[i] != b[i]) dis++;
    vbase val(n + 1);
    y_combinator(
        [&](auto self, int x, base p, base q) -> void
        {
            if (x == 0)
                return;
            base s = (p * base(x) - p * base(n) + base(n));
            base pp = base(x) / s;
            base qq = ((base(n) - base(x)) * q + base(n)) / s;

            self(x - 1, pp, qq);
            val[x] = pp * val[x - 1] + qq;
        })(n - 1, base(1), base(1));
    val[n] = val[n - 1] + base(1);
    cout << val[dis] << endl;
}
