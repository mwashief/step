/*
    09 September 2023
    Codeforces 1751-2000 1857 1857G
*/
...
public:
    ...
    DSU(int numberOfElements, int s) : n(numberOfElements), S(s)
    {
    ...
    bool join(int subset1, int subset2, int val)
    {
        ...
        res *= base(S - val + 1).pow(1LL * subsetSize[subset1] * subsetSize[subset2] - 1LL);
...
inline void __run_test()
{
    ...
    rep(i, n - 1) cin >> edges[i].sc.fr >> edges[i].sc.sc >> edges[i].fr;
    sort(a2z(edges));

    for (auto edge : edges)
        dsu.join(edge.sc.fr - 1, edge.sc.sc - 1, edge.fr);
    cout << dsu.res.x << endl;
}
