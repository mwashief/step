...
int main()
{
    ...
    auto fibonacci = y_combinator([&](auto self, int k)
                                  {
                                      if (k == 1 || k == 0)
                                          return 1;
                                      if (memo[k] != -1)
                                          return memo[k];
                                      return memo[k] = (self(k - 1) + self(k - 2)) % MOD;
                                  });
    base res = 0;
    for (int i = 1; i < n; i++)
        res += base(fibonacci(i)) * base(fibonacci(n - i)) * base(a[i]) +
               base(fibonacci(i - 1)) * base(fibonacci(n - i - 1)) * base(MOD - a[i]);
    res += base(fibonacci(n)) * base(a[0]);
    cout << res << endl;
}