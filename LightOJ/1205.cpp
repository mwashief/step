...
ll dp(int pos, int match, int zero, int isSmaller, int haveToSmaller)
{
    ...
    if (memo[pos][match][zero][isSmaller][haveToSmaller] != -1) return memo[pos][match][zero][isSmaller][haveToSmaller];
...
ll getRes(ll x)
{
    ...
    for (int i=1; i<n; i++) res+= highest[i];
    return res;
}
