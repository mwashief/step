...
{
    if (now<0) return cons(co) + taken;
    if (memo[now][alreadySmaller][co][taken] != -1) return memo[now][alreadySmaller][co][taken];
    if (alreadySmaller)
        return memo[now][alreadySmaller][co][taken] = dp(now-1, 1, co+1, taken) + dp(now-1, 1, 0, taken+cons(co));
    if ((n&(1LL<<now)) == 0) return memo[now][0][co][taken] = dp(now-1, 0, 0, taken+cons(co));
